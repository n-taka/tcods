...
namespace tcods
{
   ...
   : next( dummyHalfEdge.end() ),
     flip( dummyHalfEdge.end() ),
     from( dummyVertex.end() )
   {}

   Vertex :: Vertex( void )
   : out( dummyHalfEdge.end() ), k( 0. )
   {}
   ...
   Face :: Face( void )
   : he( dummyHalfEdge.end() ), constraintAngle( -1. )
   {}
   ...
   void Mesh :: appendDualGenerators( vector<Cycle>& cycles )
   {
      for( EdgeIter e = edges.begin(); e != edges.end(); e++ )
      {
             ...
             !inDualSpanningTree( e->he ))
         {
            ...
            int m = static_cast<int>(c1.size())-1;
            int n = static_cast<int>(c2.size())-1;
            while( c1[m] == c2[n] ) { m--; n--; }
            ...
            if( isDualBoundaryLoop( g ))
            {
               ...
               {
                  unsigned int n = static_cast<unsigned int>(g.size());
   ...
   {
      int lastDot = static_cast<int>(filename.find_last_of( '.' ));
      int extLength = static_cast<int>(filename.length()) - lastDot - 1;
   ...
   {
      return static_cast<int>(vertices.size() - edges.size() + faces.size());
   }
}
